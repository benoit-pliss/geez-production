    ...
    <header class="bg-slate-950 sticky top-0 transition-all duration-200 z-40">
      <nav class="mx-auto flex max-w-7xl items-center justify-between px-6 py-2 lg:px-8" aria-label="Global">
        ...
        <div class="hidden lg:flex lg:gap-x-12">
          <a v-for="item in navigation" :key="item.name" v-on:click="replaceTag(item.name)" class="text-sm font-semibold leading-6 hover:bg-slate-900 transition-all duration-200 py-2 px-4 rounded-lg text-white cursor-pointer select-none">{{ item.name }}</a>
          <Popover class="relative">
            <PopoverButton class="inline-flex items-center gap-x-1 text-sm font-semibold leading-6 text-white outline-none hover:bg-slate-900 py-2 px-4 rounded-lg transition-all duration-200">
              <span>Menu</span>
            ...
            <transition enter-active-class="transition ease-out duration-200" enter-from-class="opacity-0 translate-y-1" enter-to-class="opacity-100 translate-y-0" leave-active-class="transition ease-in duration-150" leave-from-class="opacity-100 translate-y-0" leave-to-class="opacity-0 translate-y-1">
              <PopoverPanel class="absolute right-0 z-10 mt-5 w-screen max-w-md px-4">
                <div class="w-screen max-w-md flex-auto overflow-hidden rounded-3xl bg-slate-950 text-sm leading-6 shadow-lg ring-1 ring-slate-900/5 m-2">
                  <div class="p-4">
                    <a v-for="item in solutions" :key="item.name" class="group relative flex gap-x-6 rounded-lg p-4 hover:bg-slate-900 transition-all duration-200" :href="item.href">
                      <div class="mt-1 flex h-11 w-11 flex-none items-center justify-center rounded-lg bg-slate-900 group-hover:bg-slate-950 transition-all duration-200">
      ...
      <Dialog as="div" class="lg:hidden" @close="mobileMenuOpen = false" :open="mobileMenuOpen">
        ...
        <DialogPanel class="fixed inset-y-0 right-0 z-50 w-full overflow-y-auto bg-slate-950 px-6 py-2 sm:max-w-sm sm:ring-1 sm:ring-gray-900/10">
