<template>
    <div class="bg-white pb-24 sm:pb-32 pt-24" id="gallery" ref="gallery">
        <div class="mx-auto px-6 lg:px-8">
            <div class="mx-auto max-w-2xl text-center">
                <h2 class="text-lg font-semibold leading-8 tracking-tight text-indigo-600">Galerie</h2>
                <p class="mt-2 text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl">Découvrez nos
                    réalisations<span v-if="props.pageTag"> - </span><span class="text-indigo-700"
                        v-if="props.pageTag">{{ props.pageTag }}</span></p>
            </div>
            ...
            <div class="border-b border-gray-200 pb-5 sm:flex sm:items-center sm:justify-between mt-10">
                <div class="flex items-start gap-y-2 flex-col">
                    ...
                    <div class="flex mt-3 sm:mt-0 gap-x-2">
                        ...
                        <Badge v-for="tag in current_tags" :key="tag.id" :label="tag.name" :color="tag.color"
                            :id="tag.id" :onClick="removeTag" type="remove" />
                        <!--Message if no tag is selected-->
                        <span v-if="current_tags.length === 0" class="text-sm font-medium text-gray-500">Aucun filtre
                            sélectionné</span>
                    </div>
                </div>
                ...
                <Combobox as="div" v-model="selectedTags">
                    <div class="relative mt-2">
                        <ComboboxInput
                            class="w-full rounded-md border-0 bg-white py-1.5 pl-3 pr-10 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6 outline-none"
                            @change="query = $event.target.value" v-on:keydown.enter.prevent="addTag(filteredTags[0])"
                        ...
                        <ComboboxOptions v-if="filteredTags.length > 0"
                            class="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
                            <ComboboxOption v-for="tag in filteredTags" :key="tag.id" :value="tag" as="template"
                                v-slot="{ active }">
                                <li :class="['relative cursor-default select-none py-2 pl-3 pr-9', active ? 'bg-gray-200' : 'text-gray-900']"
            ...
            <div class="mx-auto mt-16 flow-root max-w-2xl sm:mt-20 md:mx-0 md:max-w-none px-4">
                <div class="flex flex-wrap -mx-4 gap-4 sm:gap-10 items-start justify-center">
                    <div v-for="image in images" :key="image.name" class="w-full sm:w-auto flex-shrink-0">
                        <div class="overflow-hidden transition duration-300 transform rounded-lg sm:hover:scale-105 relative" v-on:mouseover="image.hidden = true" v-on:mouseleave="image.hidden = false">
                            <img :src="image.url" loading="lazy" class="object-contain w-auto sm:h-[30rem]" alt="geez"/>
                            <div class="absolute inset-0 flex place-content-end justify-start flex-wrap-reverse gap-2 p-4" :class="{ 'hidden' : !image.hidden }">
                                <Badge v-for="tag in image.tags" :key="tag.id" :label="tag.name" :color="tag.color" type="add" v-on:click="addTag(tag)" :id="tag.id"/>
                            </div>
                        </div>
